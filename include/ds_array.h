/**
 * @file ds_array.h
 * @brief Implementation of a generic dynamic array in C, using `void*` to hold
 * data of any type.
 */
...
/**
 * @brief Creates a new dynamic array with the specified initial capacity and
 * element size.
 *
...
/**
 * @brief Deinitializes a dynamic array, freeing its internal buffer but not the
 * array struct itself.
 *
...
/**
 * @brief Destroys a dynamic array, freeing both its internal buffer and the
 * array struct itself.
 *
 ...
 * member_size`
 *
 * @note The element is copied into the array, so the caller retains ownership
 * of the original.
 *
 * @see ds_array_pop
 * @see ds_array_insert
 ...
 * @param other_array The array from which elements will be copied. Must not be
 * NULL and must be different from `array`.
 *
 * @return DS_STATUS_OK on success
 ...
 * @return DS_STATUS_OVERFLOW if resizing the array would exceed `SIZE_MAX /
 * member_size`
 * @return DS_STATUS_MEMBER_SIZE_MISMATCH if the member sizes of the two arrays
 * do not match
 * @return DS_STATUS_ERROR if both arrays are the same
 *
 * @note The elements are copied into the array, so the caller retains ownership
 * of the originals.
 *
 * @see ds_array_push
 * @see ds_array_insert
 ...
 *
 * @param array The array to which the element will be inserted. Must not be
 * NULL.
 * @param index The index at which to insert the element. Must be a valid index
 * (0 <= index < ds_array_size(array)).
...
/**
 * @brief Retrieves a pointer to the element at a specific index in the dynamic
 * array.
 *
 * @param array The array from which to retrieve the element. Must not be NULL.
 * @param index The index of the element to retrieve. Must be a valid index (0
 * <= index < ds_array_size(array)).
 *
 * @return void* A pointer to the element at the specified index, or NULL if the
 * index is out of bounds/the array is NULL.
 *
...
/**
 * @brief Retrieves a const pointer to the element at a specific index in the
 * dynamic array.
 *
 * @param array The array from which to retrieve the element. Must not be NULL.
 * @param index The index of the element to retrieve. Must be a valid index (0
 * <= index < ds_array_size(array)).
 *
 * @return const void* A const pointer to the element at the specified index, or
 * NULL if the index is out of bounds/the array is NULL.
 *
 * @see ds_array_get
 *
 * @memberof ds_array
 ...
 * @param index The index of the element to remove. Must be a valid index (0 <=
 * index < ds_array_size(array)).
 * @param out A pointer to a buffer where the removed element will be stored, or
 * NULL if the element is not needed.
 *
 * @note If the array has a destroy function, it will be called on the removed
 * element after it is copied to `out`.
 * @note If the array's length is less than capacity / 4, the array will be
 * resized to half its current capacity to save memory.
 *
 * @warning If the array does not have a destroy function and out is set to
 ...
 *
 * @note If destroy_fn is defined, it will be called on each element before
 * clearing the array.
 *
 * @memberof ds_array
 * @ingroup array_api
...
/**
 * @brief Returns a shallow copy of the array, duplicating the internal buffer
...
/**
 * @brief Reverses the array in place, so that the first element becomes the
...
#define DS_DEFINE_ARRAY(T, name)                                               \
    ...
    static inline DS_STATUS name##_push(name* array, T value) {                \
        return ds_array_push((ds_array*)array, &value);                        \
                                                                               ...
                                                                               \
    static inline DS_STATUS name##_push_array(name* array,                     \
                                              const name* other_array) {       \
        return ds_array_push_array((ds_array*)array,                           \
                                   (const ds_array*)other_array);              \
    }                                                                          \
                                                                               \
    static inline DS_STATUS name##_insert(name* array, size_t index,           \
                                          T value) {                           \
...
static size_t ds_array_grow(size_t capacity) {
    return capacity < DS_ARRAY_MIN_CAPACITY ? DS_ARRAY_MIN_CAPACITY
                                            : capacity * 2;
}
...
DS_STATUS ds_array_init(ds_array* array, size_t initial_capacity,
    ...
    if (!array->data) {
        DS_HANDLE_FAILURE("array data initialization failed",
...
DS_STATUS ds_array_push(ds_array* array, const void* element) {
    ...
    if (array->length >= array->capacity) {
        DS_STATUS status =
            ds_array_resize(array, ds_array_grow(array->capacity));
        if (status != DS_STATUS_OK) {
            return status;
        }
    }
    memcpy((char*)array->data + (array->length * array->member_size), element,
           array->member_size);
    array->length++;
    return DS_STATUS_OK;
}
...
DS_STATUS ds_array_push_array(ds_array* array, const ds_array* other_array) {
    ...
    if (array->member_size != other_array->member_size) {
        DS_HANDLE_FAILURE("member sizes do not match",
                          DS_STATUS_MEMBER_SIZE_MISMATCH);
    }
    if (array == other_array) {
        DS_HANDLE_FAILURE("array and other_array are the same",
                          DS_STATUS_ERROR);
    }
    ...
    memcpy((char*)array->data + (array->length * array->member_size),
           other_array->data, other_array->length * array->member_size);
...
DS_STATUS ds_array_insert(ds_array* array, size_t index, const void* element) {
    ...
    memmove((char*)array->data + ((index + 1) * array->member_size),
            (char*)array->data + (index * array->member_size),
            (array->length - index) * array->member_size);
    memcpy((char*)array->data + (index * array->member_size), element,
           array->member_size);
    array->length++;
    return DS_STATUS_OK;
}
...
DS_STATUS ds_array_remove(ds_array* array, size_t index, void* out) {
    ...
    memmove((char*)array->data + (index * array->member_size),
...
DS_STATUS ds_array_pop(ds_array* array, void* out) {
    ...
    void* popped_element =
        (char*)array->data + ((array->length - 1) * array->member_size);
    if (out) {
...
DS_STATUS ds_array_reverse(ds_array* array) {
    ...
    for (size_t i = 0; i < array->length / 2; ++i) {
        memcpy(temp, (char*)array->data + (i * array->member_size),
               array->member_size);
        memcpy(
            (char*)array->data + (i * array->member_size),
            (char*)array->data + ((array->length - 1 - i) * array->member_size),
            array->member_size);
        memcpy(
            (char*)array->data + ((array->length - 1 - i) * array->member_size),
            temp, array->member_size);
    }
    free(temp);
    return DS_STATUS_OK;
}
