...
/**
 * @brief A generic stack data structure, implemented as a wrapper around a dynamic array.
 * @ingroup stack_api
 */
...
#define DS_DEFINE_STACK(T, name) \
    ...
    \
    static inline DS_STATUS name##_init(name* stack, size_t initial_capacity, ds_cmp_fn cmp_fn, ds_destroy_fn destroy_fn, ds_copy_fn copy_fn) { \
        return ds_array_init((ds_array*)stack, initial_capacity, sizeof(T), cmp_fn, destroy_fn, copy_fn); \
    } \
    \
...
/**
 * @brief Creates a new stack with the specified initial capacity and element size.
 * 
 ...
 *
 * @note The caller is responsible for freeing the returned stack using ds_stack_destroy().
 *
 ...
 * @ingroup stack_api
 */
static inline ds_stack* ds_stack_create(size_t initial_capacity, size_t member_size, ds_cmp_fn cmp_fn, ds_destroy_fn destroy_fn, ds_copy_fn copy_fn) {
    return ds_array_create(initial_capacity, member_size, cmp_fn, destroy_fn, copy_fn);
}

/**
 * @brief Initializes an existing stack with the specified initial capacity and element size.
 * 
 * @param stack The stack to initialize. Must not be NULL. 
 * @param initial_capacity The initial capacity of the stack. Must be greater than 0.
 ...
 * @ingroup stack_api
 */
static inline DS_STATUS ds_stack_init(ds_stack* stack, size_t initial_capacity, size_t member_size, ds_cmp_fn cmp_fn, ds_destroy_fn destroy_fn, ds_copy_fn copy_fn) {
    return ds_array_init(stack, initial_capacity, member_size, cmp_fn, destroy_fn, copy_fn);
}
 ...
 *
 * @note This function does not free the stack itself; it only deinitializes the internal buffer.
 *
 * @see ds_stack_destroy
 ...
 *
 * @note The element is copied into the stack, so the caller retains ownership of the original.
 *
 * @see ds_stack_pop
 ...
 *
 * @note If the stack has a destroy function, it will be called on the removed element after it is copied to `out`.
 * @note If the stack's length is less than capacity / 4, the array will be resized to half its current capacity to save memory.
 *
 * @warning If the stack does not have a destroy function and out is set to NULL, the removed element will be lost and may cause a memory leak if it was dynamically allocated.
 *
 * @return DS_STATUS_OK on success
 * @return DS_STATUS_IS_NULL if stack is NULL
